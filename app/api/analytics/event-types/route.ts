 ...
 * Get event type performance statistics for the authenticated user
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const eventTypeStats = await serverAnalyticsService.getEventTypeStats(user.id, dateRange)

    return NextResponse.json(eventTypeStats, { status: 200 })
  } catch (error) {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch event type stats' },
      { status: 500 }
    )
  }
}
