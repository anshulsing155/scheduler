 ...
 * Get revenue metrics and payment analytics for the authenticated user
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const revenueMetrics = await serverAnalyticsService.getRevenueMetrics(user.id, dateRange)

    return NextResponse.json(revenueMetrics, { status: 200 })
  } catch (error) {
